...
const SUGGESTIONS = [
  "Show me total revenue by rep this quarter",
  "Which deals are closing this month?",
  "Compare commission rates across comp plans",
  "Who is closest to hitting their quota?",
];
  ...
  return (
    <AppLayout>
      <div className="flex flex-col h-[calc(100vh-7rem)] sm:h-[calc(100vh-8rem)]">
        ...
        <div
          ref={scrollRef}
          className="flex-1 overflow-y-auto border border-border bg-muted/30 p-4 sm:p-6"
        >
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-6">
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <h2 className="text-lg font-semibold">CommCalc AI</h2>
                <p className="text-sm text-muted-foreground max-w-md">
                  Ask anything about your deals, reps, commissions, or comp plans. I have access to your live data.
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-lg w-full">
                {SUGGESTIONS.map((s) => (
                  <button
                    ...
                    className="text-left text-sm px-3 py-2.5 border border-border bg-card hover:bg-accent transition-colors"
                  >
                    {s}
                  </button>
                ))}
